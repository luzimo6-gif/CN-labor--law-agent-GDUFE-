...
# DeepSeek 配置（用于对话 LLM）
deepseek_key = st.secrets.get("DEEPSEEK_API_KEY") or os.getenv("DEEPSEEK_API_KEY", "")
deepseek_url = st.secrets.get("DEEPSEEK_BASE_URL") or os.getenv("DEEPSEEK_BASE_URL", "https://api.deepseek.com")
...
_masked_key = deepseek_key[:8] + "..." + deepseek_key[-4:] if len(deepseek_key) > 12 else "(KEY太短或为空)"
print(f">>> DeepSeek Key: {_masked_key}")
print(f">>> DeepSeek URL: {deepseek_url}")
print(f">>> Embedding: DashScope ({api_key[:8]}...{api_key[-4:]})")
...
try:
    ...
    print(f">>> DeepSeek 连通性: HTTP {_test_resp.status_code}")
except Exception as _e:
    print(f">>> ⚠️ DeepSeek 连接测试失败: {type(_e).__name__}: {_e}")

# ==========================================
# 1.5 自动模型回退包装器（余额不足自动换模型）
# ==========================================
DEEPSEEK_MODELS = ["deepseek-chat"]

class AutoFallbackLLM:
    """当余额不足时自动尝试下一个模型"""
    def __init__(self, models, api_key, base_url, temperature, max_tokens):
        self.models = models
        self.api_key = api_key
    ...
    def _try_invoke(self, method_name, *args, **kwargs):
        last_error = None
        for i, model in enumerate(self.models):
            try:
                ...
                err_str = str(e)
                if "FreeTierOnly" in err_str or "InsufficientBalance" in err_str or "余额" in err_str:
                    last_error = e
                    next_model = self.models[i+1] if i+1 < len(self.models) else "无可用模型"
                    print(f">>> 模型 {model} 余额不足，切换 {next_model} ...")
                    continue
                raise
...
llm = AutoFallbackLLM(
    models=DEEPSEEK_MODELS, api_key=deepseek_key, base_url=deepseek_url,
